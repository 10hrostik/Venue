let transformFilterPane = {
    setToForm: function(criteria, type, objectType) {
                    if(criteria != null) {
                        ...
                        if(criteria.genresToSearch != null) {
                            for(let i = 0; i < criteria.genresToSearch.length; i++) {
                                for(let j = 0; j < genres.length; j++) {
                                    if(genres[j].id == criteria.genresToSearch[i]) {
    ...
    transformGenreCriteria: function(genres, objectType) {
        ...
        for(let genre of genres) {
            inputs.push(<input key={"input" + iterator} className={"genre" + objectType}  style={{border: "4px solid black"}} type="checkbox" id={genre} name={genre + " genre"}/>)
