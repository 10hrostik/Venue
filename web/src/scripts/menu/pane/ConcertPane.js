...
export default function ConcertPane(props) {
    ...
    useEffect(() => {
        ...
        .then((fetchedData) => {
            setGenres(transformFilterPane.transformGenreCriteria(fetchedData, "Concert"));
        })
        .catch((error) => {
            console.log(error);
        });
    ...
    useEffect(() => {
        if(criteria != null && props.currentType == 'CONCERT') {
            let settings = JSON.parse(criteria.concert);
    ...
    const handleDefault = (event) => {
        ...
        transformFilterPane.setDefaultFilter(props.type, "Concert")
        fetchDefaultEvent();
        if(user != null) {
            saveDefaultCriteria();
        }
